...
public class LeapWebProcessor : MonoBehaviour
{
    ...
    private Hand creatHand(long frameId, object hand, object pointables_object)
    {
        ...
        Hand result = new Hand(frameId, (int)Handid, (float)confidence, (float)grabStrength, 0, (float)pinchStrength, 0, (float)s, isLeft, (float)timeVisible, arm, fingerlist, palmPosition, stabilizedPalmPosition, palmVelocity, palmNormal, new LeapQuaternion(), direction, wrist);
